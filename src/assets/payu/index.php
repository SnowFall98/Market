	...
	if(isset($_POST["extra1"])){
		...
		foreach ($idProducts as $key => $value) {
			...
			curl_setopt_array($curl, array(
			  CURLOPT_URL => "https://market-place-363dc-default-rtdb.firebaseio.com/products/".explode(",",$value)[0].".json",
	...
	if(isset($_POST["extra2"])){
		...
		foreach ($idOrders as $key => $value) {
			...
			$curl = curl_init();
			  ...
			  curl_setopt_array($curl, array(
			  CURLOPT_URL => "https://market-place-363dc-default-rtdb.firebaseio.com/orders/".$value.".json",
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => "",
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
	 ...
	 if(isset($_POST["extra3"])){
		...
		foreach ($idSales as $key => $value) {
			...
			$curl = curl_init();
			  ...
			  curl_setopt_array($curl, array(
			  CURLOPT_URL => "https://market-place-363dc-default-rtdb.firebaseio.com/sales/".$value.".json",
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => "",
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
